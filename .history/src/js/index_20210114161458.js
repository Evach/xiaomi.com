...
$.get('./html/public/header.html',function(data){
    ...
    $.get("../interface/tab.json",function(json){
        let temp;
        json.each((i,item)=>{
            let $li="";
            item.data.each((index,elm)=>{
                $li+=`
                <li>
                    <a href="#">
                        <div class="pic"><img src="${elm.src}" alt=""></div>
                        <h4 class="title">${elm.title}</h4>
                        <p class="price">${elm.price}元</p>
                    </a>
                </li>
                `;
            })
            temp = `
            <div class="hclink_menu_outer">
                    <div class="contain">
                        <ul class="hclink_menu clearfix">
            `
        })
    })
})
