...
function setnum(elm) { //设置购买产品数量
    let $val = parseInt(elm.siblings("input").val()
    if(elm.attr(id) == "add"){
        elm.siblings("input").val(val++);
    }
    if(elm.attr(id) == "reduce"){

        elm.siblings("input").val(parseInt(elm.siblings("input").val())--);
    }
