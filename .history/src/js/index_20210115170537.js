...
var timeDelay = setInterval(function () {
    var now = new Date();
    var futuer = new Date(2021, 0 , 1, 0, 0, 0);
    var getTime = Math.floor((futuer - now) / 1000);
    var _time = document.getElementById("time");
    var _timeHtml = "";
    var dd = Math.floor(getTime / 60 / 60 / 24); 
    var hour = Math.floor(getTime / 60 / 60 % 24);
    var minutes = Math.floor(getTime / 60 % 60) < 10 ? 0 + Math.floor(getTime / 60 % 60) : Math.floor(getTime / 60 % 60);
    var seconds = Math.floor(getTime % 60) < 10 ? 0 + Math.floor(getTime % 60) : Math.floor(getTime % 60);
    ...
    _time.innerHTML = _timeHtml.concat("距离2021年1月1日0点0分0秒  还剩", dd, "天", hour, "小时", minutes, "分钟", seconds, "秒");
