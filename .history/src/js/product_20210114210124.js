...
function addGoods(id,versions,color,price,num){//optipns 包含 版本颜色价格数量
    ...
    if(goodsData){
        ...
        if(goodsData.some(elm => elm.id === id&&elm.versions===versions&&elm.color===color)){//选择相同的产品
            ...
            // })
            let newGoods = goodsData.forEach((elm)=>{
                let res = []
                if(elm.id === id&&elm.versions==versions&&elm.color===color){
                    elm.num+=1;
                }
               
            })
            console.log(newGoods)
            cookie.set("goods",JSON.stringify(newGoods),1);
