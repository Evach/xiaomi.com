...
function addGoods(id,options){//optipns b包含 版本颜色jia'ge
    const goods ={
        id,
        type:options
    }
    ...
    if(goodsData){
        ...
        if(goodsData.some(elm => elm.id === id)){
            if(goodsData.type.some(elm => elm.versions === versions)){

            }else{
                goodsData.forEach(elm => elm.id === id?elm.type = num:null);
            }
