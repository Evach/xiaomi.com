...
function addGoods(id,versions,color,price,){//optipns 包含 版本颜色价格数量
    const goods = {
        id,
        options
    };
    ...
    if(goodsData){
        ...
        if(goodsData.some(elm => elm.id === id&&elm.versions===options.versions&&elm.color===options.color)){//选择相同的产品
            ...
            // })
            goodsData.filter(elm => elm.id === id&&elm.versions===options.versions&&elm.color===options.color)
        }else{
