...
if(goodsData){
    ...
    idList.length!=0 && $(".sum_btn").addClass("active");
    $(".cart_main[name='empty']").hide().siblings(".cart_main").show();
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            data.forEach(elm => {
                ...
                typeArr.forEach(e => {
                    let sum = parseFloat(e.price*e.num).toFixed(1);
                    temp += `
                        <tr >
                            ...
                            <td>
                                <div class="change_num clearfix">
                                    <a href="javascript:void(0)" class="reduce"><i class="iconfont">-</i></a>
                                    <input type="text" value="${e.num}" class="goods_num" >
                                    <a href="javascript:void(0)" class="add"> <i class="iconfont">+</i></a>
        ...
        if(res){
            ...
            $(".icon_check").on("click",function(){
                let that=$(this);
                let checkArr = $(".icon_check");
                if(that.attr("data-flag")){
                    if(that.attr("name")=="all"){
                        checkArr.removeClass("active").attr("data-flag",null);
                        $(".sum_btn").removeClass("active");
                    }else{
                        that.removeClass("active").attr("data-flag",null);
                    }
                    let only = $(".icon_check[data-flag=1]");
                    if(only.length==1&&only.attr("name")=="all"){
                        only.removeClass("active").attr("data-flag",null);
                        $(".sum_btn").removeClass("active");
                    }
                    
                }else{
                    if(that.attr("name")=="all"){
                        checkArr.addClass("active").attr("data-flag",1);
                    }else{
    ...
    $(".cart_main[name='empty']").show().siblings(".cart_main").hide();
}
...
function setnum() { //设置购买产品数量
    ...
    $th.find("strong").html((parseFloat($th.find(".univalence").html())*$val).toFixed(1)+"元");
    calculate();
    setCook($(this));
