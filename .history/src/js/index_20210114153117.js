...
$.get('./html/public/header.html',function(data){
    ...
    tabTurn(".hc_nav li a",".hclink_menu_outer");
});
...
function tabTurn(nav,navmenu){
    $(nav).hover(
        function(){
            $(nav).siblings(navmenu).addClass("active").parent().siblings().find(navmenu).removeClass("active");
        },
        function(){
            $(nav).parent().parent().find(navmenu).removeClass("active");
        }
    )
}