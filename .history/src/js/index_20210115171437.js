...
let timeDelay = setInterval(function () {
    ...
    let hour = Math.floor(getTime / 60 / 60 % 24) < 10 ? "0" + Math.floor(getTime / 60 / 60 % 24) : Math.floor(getTime / 60 / 60 % 24);
    ...
    if (futuer - now <= 999) {
        ...
        _timeHtml=`
        <span>00</span><i>:</i><span>${minutes}</span><i>:</i><span>${seconds}</span>
        `;
